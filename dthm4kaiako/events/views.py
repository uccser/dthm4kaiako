...
class EventDetailView(RedirectToCosmeticURLMixin, generic.DetailView):
    ...
    def does_registration_exist_provided_logged_in(self, user):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            if self.does_registration_exist_provided_logged_in(user):
...
def create_new_participant_type_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if participant_type_creation_form.is_valid():
            ...
            messages.warning(
                request,
                'Please resolve the invalid fields to create a new event participant type.'
            )
